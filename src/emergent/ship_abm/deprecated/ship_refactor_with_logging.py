...
class FossenShip:
    def __init__(self, ID, position, heading, speed, length, beam, draft, max_rudder_angle=30):
        self.ID = ID
    ...
    def calculate_thrust(self):
        ...
        logging.debug(f"Vessel {self.ID}: Thrust = {thrust_force:.2f}")
    ...
    def adjust_rpm(self, desired_speed, dt):
        ...
        logging.debug(f"Vessel {self.ID}: Desired Speed = {desired_speed:.2f}, Current Speed = {self.nu[0]:.2f}, "
    ...
    def pid_control(self, desired_heading, dt):
        heading_error = (desired_heading - self.current_heading) % 360
        if heading_error > 180:  # Handle shortest direction
            heading_error -= 360
    
        proportional = self.Kp * heading_error
        self.integral += heading_error * dt
        integral = self.Ki * self.integral
        derivative = self.Kd * (heading_error - self.previous_error) / dt
        self.previous_error = heading_error
    
        rudder_angle = proportional + integral + derivative
        rudder_angle = max(min(rudder_angle, self.max_rudder_angle), -self.max_rudder_angle)  # Clamp rudder angle
    
        # Debugging the PID terms
        #log(f"PID Debug | Heading Error: {heading_error:.2f}, Proportional: {proportional:.2f}, Integral: {integral:.2f}, Derivative: {derivative:.2f}, Rudder Angle: {rudder_angle:.2f}")
        self.current_heading += rudder_angle * dt  # or any appropriate logic
        self.current_heading %= 360  # Normalize to [0, 360) degrees
        log.debug(f"[Vessel {self.ID}] PID: Heading error: {heading_error}, Rudder angle: {rudder_angle}")
        ...
        for other in other_vessels:
            ...
            if closing_speed < 0:  # Approaching
                ...
                # Head-on scenario (within ±10° ahead)
                    ...
                    priority = 3
        log.debug(f"[Vessel {self.ID}] COLREGS: Current heading: {self.current_heading}, Desired heading: {avoidance_heading}")
    ...
    def update(self, tau, wind, current, dt, desired_speed):
        ...
        logging.debug(f"Vessel {self.ID}: Thrust = {thrust[0]:.2f}, Drag = {drag[0]:.2f}, "
        ...
        log.debug(f"[Vessel {self.ID}] Update: Position: {self.currentPos}, Heading: {self.current_heading}, Speed: {self.nu[0]}")

        # Debug position and speed
        logging.debug(f"Vessel {self.ID}: Position = {self.currentPos}, Speed = {self.nu[0]:.2f}, "
...
class Simulation:
    def __init__(self, vessels, wind, current, dt=0.1, simulation_time=100):
        ...
        logging.debug(f"Trajectories Initialized: {self.trajectories}")
        # Visualization setup
    ...
    def _update_plot(self, frame):
        logging.debug(f"Frame {frame} updating...")
        self._update_vessels()  # Update all vessels' states
    
        for vessel in self.vessels:
            logging.debug(f"Frame {frame}, Vessel {vessel.ID} Position: {vessel.currentPos}")
            self.plots[vessel.ID].set_data(
    ...
    def run(self):
        ...
        while True:
            for vessel in self.vessels:
                ...
                # Debug output
                logging.debug(f"Vessel {vessel.ID}: Position = {vessel.currentPos}, "
    ...
    def get_trajectories(self):
        ...
        for key in self.trajectories:
            ...
            logging.debug(f"Vessel {key} trajectory: {result[key]}")  # Debug output
        return result
...
sim = Simulation(vessels, wind, current, dt=0.1, simulation_time=1)
sim.run()
