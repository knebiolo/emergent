...
#     with h5py.File(HECRAS_dir + '.hdf', 'a') as output_hdf:
#         pts = np.array(output_hdf.get('Geometry/2D Flow Areas/2D area/Cells Center Coordinate'))
#         elev = np.array(output_hdf.get('Geometry/2D Flow Areas/2D area/Cells Minimum Elevation'))
#         for timestep in timestep_range:
#             print(f"Processing timestep: {timestep}")
            
#             vel_x = output_hdf['Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/2D Flow Areas/2D area/Cell Velocity - Velocity X'][timestep]
#             vel_y = output_hdf['Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/2D Flow Areas/2D area/Cell Velocity - Velocity Y'][timestep]
#             wsel = output_hdf['Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/2D Flow Areas/2D area/Water Surface'][timestep]
...
class simulation():
    ...
    def __init__(self, 
        ...
        prolonged_swim_speed_adjustment = 6.7
        ...
        try:
            ...
            # dict-like fallback for test environments
            self.hdf5 = {}
        ...
        # write agent properties that do not change with time using hdf5_io
        h5obj = hdf5_io.get_hdf5_obj(self) or self.hdf5
        ...
        hdf5_io.write_dataset(h5obj, "agent_data/opt_wat_depth", self.sex)
    ...
    def odometer(self, t, dt):
        ...
        body_depth (array): Body depth of each fish.
        submergence_depth (array): Submergence depth of each fish.
        ...
        - The function updates the kcal attribute for each fish based on their oxygen
          ...
          and adjustments based on Hughes (2004) for environmental factors.
        """
    
        # Calculate standard metabolic rate (SMR) using equations from Brett (1973)
        smr_o2_rate = self.arr.where(
            self.water_temp <= 5.3,
            self.arr.exp(0.0565 * self.arr.power(self.arr.log(self.weight), 0.9141)),
            self.arr.where(
                self.water_temp <= 15,
        ...
        # Calculate active metabolic rate (AMR) using equations from Brett (1973)
        amr_o2_rate = self.arr.where(
            self.water_temp <= 5.3,
        ...
        # Calculate wave drag adjustment based on Hughes (2004)
        # Wave drag is a function of the ratio of body depth to submergence depth
        drag_multiplier = self.body_depth / self.submergence_depth
        wave_drag_effect = np.clip(drag_multiplier, 1.0, 5.0)  # Clip values based on Hughes (e.g., max 5x drag)
    
        # Calculate combined metabolic rate based on swim speed and wave drag
        swim_factor = self.swim_speed / self.ucrit  # Swimming speed as a fraction of critical speed
        combined_o2_rate = smr_o2_rate + (amr_o2_rate - smr_o2_rate) * swim_factor * wave_drag_effect
    
        # Convert the oxygen consumption rate to mg O2 per kg over the time step (dt in seconds)
        hours = dt * (1./3600.)  # Convert dt to hours
        per_capita_swim_cost = combined_o2_rate * hours
        mg_O2 = per_capita_swim_cost * self.weight  # Scale to the fish's weight
    
        # Convert oxygen consumption to kilocalories
        kcal_burned = mg_O2 * (3.36 / 1000)
    
        if np.any(kcal_burned < 0):
            print('Warning: negative kcal value detected.')
    
        # Update the cumulative kilocalories burned
        self.kcal += kcal_burned
    ...
    class behavior():
        ...
        def arbitrate(self,t):
            ...
            if self.simulation.pid_tuning == True:
                ...
                alignment = self.alignment_cue(9000)     # 20500
                cohesion = self.cohesion_cue(11000)       # 11000
                low_speed = self.vel_cue(7500)             # 500 
...
class summary:
    ...
    def Kcal_histogram_by_timestep_intervals_for_all_simulations(self):
        ...
        plt.xlabel("Total calories", fontsize=6)
    ...
    def kaplan_curve(self, shapefile_path, tiffWS, inputWS):
        ...
        gate_files = ['Gate_04.shp', 'Gate_03.shp', 'Gate_02.shp', 'Gate_01.shp']
