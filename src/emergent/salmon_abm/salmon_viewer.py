            ...
            def _on_mesh(payload):
                ...
                try:
                    ...
                    if preview_img is not None:
                        ...
                        # No preview could be generated by GL or FBO renderer.
                        # Skip matplotlib fallback to avoid heavyweight imports in headless environments.
                        preview_img = None
                except Exception as e:
                    print('[TIN] preview generation failed:', e)
                    # If we have a QImage, save it; otherwise skip (avoid matplotlib)
                    try:
                        if preview_img is not None:
                            out_dir = getattr(self.sim, 'model_dir', None) or '.'
                            out_path = os.path.join(out_dir, 'outputs', 'tin_preview.png')
