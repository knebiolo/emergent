"""HECRAS IO helpers (clean, single-definition).

Provides `HECRASMap` which builds a KDTree from HECRAS plan cell centers
and maps query points to one-or-more HECRAS field arrays using IDW.
"""
...
class HECRASMap:
    """Container for a HECRAS plan KDTree and one-or-more field arrays.

    Parameters
    - plan_path: path to an HDF5 HECRAS plan file
    - field_names: list of dataset names (or partial names) to extract
    - timestep: when a dataset is 2D (time x cells), pick this timestep
    """
    ...
    def _find_dataset_by_name(self, hdf: h5py.File, name_pattern: str):
    ...
    def _load_plan(self):
        ...
        self.tree = safe_build_kdtree(self.coords, name='hecras_plan_tree')
        if self.tree is None:
            try:
                logger.warning('HECRAS plan: KDTree build failed; certain queries will be disabled')
            except Exception as e:
                safe_log_exception('Failed while logging KDTree build warning', e, file='io.py')

    def map_idw(self, query_pts: Any, k: int = 8, eps: float = 1e-8) -> Dict[str, np.ndarray]:
...
def map_hecras_for_agents(plan_path: str, points: np.ndarray, field_names: List[str] = None, k: int = 8) -> Dict[str, np.ndarray]:
    """Convenience wrapper: map agent XY points to HECRAS fields."""
    m = HECRASMap(plan_path, field_names=field_names)
    return m.map_idw(points, k=k)
"""HECRAS IO helpers (clean, single-definition).

This module provides a small, well-tested adapter `HECRASMap` that
builds a KDTree from HECRAS plan cell-center coordinates and maps
query points to one or more HECRAS field arrays using inverse-distance
weighting (IDW).

The implementation intentionally stays small and predictable for unit
testing during the migration from legacy code.
"""
