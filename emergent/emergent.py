...
def initialStates (n,obstacles,origins,destinations,ships,travel_network,frames,outputWS = None):
    ...
    for i in profiles:
        ...
        if shipTypes[i] == 'Cargo':
            ...
            '''we need a comprehensive database of ship parameters,
            these values suck and I feel like we can't properly calibrate'''
            if L[i] > 253:
...
class Ship():  # create a class object to describe a self agent
    ...
    def __init__(self,
                 ...
                 v1,
                 dBase,
                 route_obstacles = None,
                 nav_obstacles = None):
        '''at initialization, modeler must provide the following parameters:
        ...
        # set ship shape parameters for length, beam and draught
        self.L = L
        self.B = B
        self.T = T

        # set dead weight tonnage
        self.DWT = DWT

        # when is this agent introduced
        self.t_start = t_start

        # what is the block coefficient of the hull
        self.C_B = 0.80
        ...
        # set propeller diameter
        self.d = 0.65 * self.T

        # convert to mass for physics
        self.m = self.DWT * 907.185

        # propeller thrust coefficient
        self.K_t = 1.2

        # calculate wetted area
        self.A = ((self.L * self.B) + (2 * (self.B * self.T)) + (2 * (self.L * self.T))) * self.C_B

        # set initial rudder heading
        self.delta_c = np.array([0.0])

        # set initial rotational velocity of vessel
        self.r = np.array([0.0])

        # get current position
        self.currentPos = np.array([self.xpos,self.ypos,0])

        # set velocity
        self.u = v0 * 0.514444
        self.startVel =v0 * 0.514444
        self.desVel = v1 * 0.514444
        self.openWaterDesVel = v1 * 0.514444
...
class simulation():
    ...
    def __init__(self,
                 ...
                 bounding_box, # model domain boundaries, in UTM [(Emin,Ymax),(Emax,Ymin)]
                 epsg, # projected coordinate system EPSG number
                 obs = None,   # shapefile of obstacles
        ...
        self.xmin = bounding_box[0][0]
        self.xmax = bounding_box[1][0]
        self.ymin = bounding_box[0][1]
        self.ymax = bounding_box[1][1]

        ''' with a bounding box, we can extract shoreline using cartopy and use
        shapely to perform the same operations on our land obstacles that
        we do in a GIS'''

        # Create List of Nodes consisting of origins and destinations
        ...
        if random_profiles == True:
            # create a travel network where any destination is accessible from any origin
            ...
            # create profiles
            self.profiles = initialStates(n_agents,
                                          obs,
                                          origins,
                                          destinations,
                                          ships,
                                          travel_network,
                                          n_frames)
        # if it does not, import profiles from directory
        #else:
            # self.profiles = pd.read_csv

        # create agent list by iterating over dataframe of profiles
        self.agents = []
        rows = np.arange(0,len(self.profiles) - (len(self.profiles)-n_agents),1)
        for i in rows:
            agn = Ship(i,self.profiles,self.resultsDB, obs)
            agn.Route()
            self.agents.append(agn)
        del agn

        # if obstacles were passed
        if obs is not None:
            ...
            for i in obs.iterrows():
                if i[1]['type'] != 'land':
                    ...
                    farm = i[1]['shape']
                    coords = list(farm.exterior.coords)
                    polygon = Polygon(coords,color = 'b', fill = True, closed = True)
                    self.farms.append(polygon)
        ...
        ax.text(self.xmin + 100,self.ymin + 100,'time: ', fontsize = 8)

        self.fig = fig

    def chaos (self, i):
        ''' the chaos method performs the agent based simulation, solving movement
        on a time-step by time-step basis and writing to project database'''
        patches = []
        agentsL = copy.copy(self.agents)
        ...
        for agent in agentsL:
            # identify next waypoint for this agent
            agent.nextWpt()
            # calculate destination mass at every time step
            agent.M(self.agents,self.obs,self.obsW)
            # calculate attractive force acting on current agent
            att = agent.F_att(self.agents, self.obs, self.obsW)
            # calculate repulsive force generated by other agents
            rep = agent.F_rep_agn(agentsL, i)
            # calculate repulsive force generated by obstacles
            obs = agent.F_rep_obs(self.windFarms,self.obsW)
            # sum total force felt by agent
            totForce = att + rep + obs
            # calculate unit vector of total force vector
            tot1Dir = totForce/np.linalg.norm(totForce)
            # calculate change in heading
            agent.delta_c = np.arctan2(tot1Dir[1],tot1Dir[0]) - agent.psi
            # solve and set necessary propeller speed
            RPS = agent.RPScommand()
            agent.RPS = RPS
            # calculate surge force acting on agent
            agent.surge()
