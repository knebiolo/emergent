...
plt.rcParams['animation.ffmpeg_path'] = 'Q:\Internal_Data\Staff_Projects\KPN\ffmpeg-master-latest-win64-gpl\bin\ffmpeg.exe'
...
class RasterCache:
    ...
    def get(self, raster_type, raster_path = None):
        if raster_type in self.cache:
            return self.cache[raster_type]
        else:
            if len(self.cache) >= self.max_size:
                self.cache.pop(next(iter(self.cache)))  # Remove the first added item
            with rasterio.open(raster_path) as src:
                array = src.read(1)  # Assuming you're reading the first band
                transform = src.transform
                width = src.width
                height = src.height
            self.cache[raster_type] = (array, transform, width, height)
            return self.cache[raster_type]

    def read_raster(self, filepath):
        # Implement your raster reading logic here
        with rasterio.open(filepath) as dataset:
            return dataset.read()

raster_cache = RasterCache(maxsize=99)  # Adjust maxsize as needed
...
class fish():
    ...
    def __init__(self, ID, model_dir, starting_block, EPSG, basin, water_temp):
        ...
        self.batt_dict = {}
        self.odometer_dict = {}
        # self.hdf_path = os.path.join(model_dir,'agent_%s.h5'%(ID))
        # hdf = pd.HDFStore(self.hdf_path)
        # hdf.close()
    ...
    def morphological_parameters(self):
        ...
        # write to database
        # hdf = pd.HDFStore(self.hdf_path)
        # hdf['agent'] = pd.DataFrame.from_dict({'ID':[self.ID],
        #                                             'sex':[self.sex],
        #                                             'length':[self.length],
        #                                             'weight':[self.weight],
    ...
    def initial_heading (self, vel_dir_tup):
        ...
        #row, col = geo_to_pixel(x, y, vel_dir_tup[1])

        # get vel_dir raster
        vel_dir = rasterio.MemoryFile()

        with vel_dir.open(
            driver ='GTiff',
            height = vel_dir_tup[0].shape[0],
            width = vel_dir_tup[0].shape[1],
            count =1,
            dtype ='float32',
            crs = self.EPSG,
            transform = vel_dir_tup[1]
        ) as dataset:
            dataset.write(vel_dir_tup[0], 1)

            # get row and column of agent pos
            row, col = dataset.index(x, y)

            # flow direction
            flow_dir = dataset.read(1)[row, col]
        vel_dir.close()
        # set direction
        if flow_dir < 0:
    ...
    def mental_map (self, depth_tup, t = None):
        ...
        with depth.open(
            driver ='GTiff',
            height = depth_tup[0].shape[0],
            width = depth_tup[0].shape[1],
            count =1,
            dtype ='float32',
            crs = self.EPSG,
            transform = depth_tup[1]
        ) as dataset:
            dataset.write(depth_tup[0], 1)
            ...
            if self.map is None:
                ...
                self.map = np.memmap(os.path.join(self.model_dir, 'agent_%s_mental_map.dat'%self.ID),
                                     dtype = 'float32',
                                     mode = 'w+',
                                     shape = depth_tup[0].shape)
    ...
    def mental_map_export(self, depth_tup):
    ...
    def already_been_here(self, transform, weight, t):
        ...
        # get depth raster
        # global raster_cache
        # depth_rast = raster_cache.get('depth')[0]
        # transform = raster_cache.get('depth')[1]
    ...
    def environment(self, depth_tup, x_vel_tup, y_vel_tup, agents_df):
        '''method finds the current depth, x velocity, y velocity, and neighbors'''
        # get current position
        x, y = (self.pos[0],self.pos[1])
        row, col = geo_to_pixel(x, y, depth_tup[1])

        # create depth, x_vel, and y_vel rasters in memory
        depth = rasterio.MemoryFile()
        x_vel = rasterio.MemoryFile()
        y_vel = rasterio.MemoryFile()
        ...
        with depth.open(
            ...
            dataset.write(depth_tup[0], 1)
            self.depth = dataset.read(1)[row, col]

        with x_vel.open(
            driver ='GTiff',
            height = x_vel_tup[0].shape[0],
            width = x_vel_tup[0].shape[1],
            count =1,
            dtype ='float32',
            crs = self.EPSG,
            transform = x_vel_tup[1]
        ) as dataset:
            dataset.write(x_vel_tup[0], 1)
            self.x_vel = dataset.read(1)[row, col]

        with y_vel.open(
            driver ='GTiff',
            height = y_vel_tup[0].shape[0],
            width = y_vel_tup[0].shape[1],
            count =1,
            dtype ='float32',
            crs = self.EPSG,
            transform = y_vel_tup[1]
        ) as dataset:
            dataset.write(y_vel_tup[0], 1)
            self.y_vel = dataset.read(1)[row, col]
    ...
    def vel_cue (self, vel_mag_tup, weight):
        ...
        lowest velocity'''
        # get vel_mag_rast
        vel_mag = rasterio.MemoryFile()

        with vel_mag.open(
            driver ='GTiff',
            height = vel_mag_tup[0].shape[0],
            width = vel_mag_tup[0].shape[1],
            count =1,
            dtype ='float32',
            crs = self.EPSG,
            transform = vel_mag_tup[1]
        ) as dataset:
            dataset.write(vel_mag_tup[0], 1)
            ...
            if self.swim_mode == 'refugia':
                ...
                # create wedge looking in front of fish
                theta = np.radians(np.linspace(-60,60,100))
    ...
    def rheo_cue (self, vel_dir_tup, weight):
        ...
        # get vel_dir_rast
        vel_dir_rast = rasterio.MemoryFile()

        with vel_dir_rast.open(
            driver ='GTiff',
            height = vel_dir_tup[0].shape[0],
            width = vel_dir_tup[0].shape[1],
            count =1,
            ...
            vel_dir = dataset.read(1)[row, col] - np.radians(180)
    ...
    def shallow_cue(self, depth_tup, weight):
        '''
        ...
        '''
        # get depth_rast
        depth_rast = rasterio.MemoryFile()

        with depth_rast.open(
            driver ='GTiff',
            height = depth_tup[0].shape[0],
            width = depth_tup[0].shape[1],
            count =1,
            ...
            #TODO - use numpy to make faster
            # create a sensory wedge looking in front of fish
            theta = np.radians(np.linspace(-90,90,100))
            arc_x = self.pos[0] + (4. * self.length/1000.) * np.cos(theta)
            arc_y = self.pos[1] + (4. * self.length/1000.) * np.sin(theta)
            arc_x = np.insert(arc_x,0,self.pos[0])
            arc_y = np.insert(arc_y,0,self.pos[1])
            arc = np.column_stack([arc_x, arc_y])
            arc = Polygon(arc)
        ...
        # if np.linalg.norm(np.nansum(x_force)) != 0.0:
        #      print ("let's check")
    ...
    def wave_drag_cue(self, depth_tup, weight):
    ...
    def collision_cue(self, weight):
        ...
        if not self.collision_neighbors.empty:
            ...
            if self.collision_neighbors['distance'].min() == 0:
                ...
                #print(c_hat)
                collision_cue = (weight * c_hat)/np.power(closest_fish['distance'].values[0],2)

                return collision_cue
        else:
            return np.array([0.,0.])

    def arbitrate(self, depth_tup, x_vel_tup, y_vel_tup, vel_dir_tup, vel_mag_tup, t):
        ...
        rheotaxis = self.rheo_cue(vel_dir_tup,10000)
        shallow = self.shallow_cue(depth_tup,15000)
        wave_drag = self.wave_drag_cue(depth_tup, 8000)
        low_speed = self.vel_cue(vel_mag_tup, 12000)
        avoid = self.already_been_here(depth_tup[1], 8000, t)
        ...
        # self.arb_df = pd.DataFrame.from_dict(row_dict, orient = 'columns')

        self.arb_dict[t] = {'ID':[self.ID],
                        'rheotaxis':['{:50}'.format(str(tuple(np.round(rheotaxis,2))))],
                        'shallow':['{:50}'.format(str(tuple(np.round(shallow,2))))],
                        'wave_drag':['{:50}'.format(str(tuple(np.round(wave_drag,2))))],
                        'low_speed':['{:50}'.format(str(tuple(np.round(low_speed,2))))],
    ...
    def swim(self, dt, t):
        ...
        # self.swim_df = pd.DataFrame.from_dict(swim_dict, orient = 'columns')

        self.swim_dict[t] = {'ID':[self.ID],
                         'speed_over_ground':[np.round(self.sog,2)],
                         'swim_speed':[np.round(self.swim_speed,2)],
                         'water_velocity':[np.round(np.linalg.norm([self.x_vel, self.y_vel]),2)],
                         'water_depth':[np.round(self.depth,2)],
    ...
    def jump (self,t):
        ...
        self.jump_dict[self.ID] = {'timestep':[t],
                         'jump_angle':[np.degrees(jump_angle)],
                         'time_airborne':[np.round(time_airborne,2)],
                         'displacement':[np.round(displacement,2)]}

        # self.jump_df = pd.DataFrame.from_dict(jump_dict, orient = 'columns')
    ...
    def fatigue(self,t):
        ...
        # self.batt_df = pd.DataFrame.from_dict(batt_dict)

        self.batt_dict[t] = {'timestep':[t],
                     'battery':[np.round(self.battery,4)],
                     'swim_mode':['{:50}'.format(self.swim_mode)],
                     'swim_behavior':['{:50}'.format(self.swim_behav)],
                     'bout_no':[self.bout_no],
    ...
    def odometer(self,t):
        ...
        # self.odo_df = pd.DataFrame.from_dict(odo_dict)

        self.odometer_dict[t] = {'timestep':[t],
                            'kcal':[np.round(swim_cost,4)],
                            'total_kcal':[np.round(self.kcal,4)]}

    def timestep(self, agents, t, dt, pos_dict, depth_tup, x_vel_tup, y_vel_tup, vel_dir_tup, vel_mag_tup):
        """
        method compiles everything required for a timestep
        ...
        self.mental_map(depth_tup, t = t)
        self.environment(depth_tup, x_vel_tup, y_vel_tup, agents_df = agents)
        ...
        self.arbitrate(depth_tup, x_vel_tup, y_vel_tup, vel_dir_tup, vel_mag_tup, t = t)

        '''if the ratio to ideal speed over ground to water velocity
        is less than 0.05
        and the agent is travelling against flow
        and it's been more than 180 seconds since its last jump
        and there is more than 40% remaining in battery'''
        if self.ideal_sog / np.linalg.norm([self.x_vel,self.y_vel]) < 0.05 and\
            np.sign(self.heading) != np.sign(np.arctan2(self.y_vel,self.x_vel)) and\
                t - self.time_of_jump > 180 and self.battery > 0.4:
                    # jump
                    self.jump(t = t)
        else:
        ...
        # write to position dictionary
        pos_dict[self.ID] = np.array([self.pos[0],self.pos[1],self.heading,self.sog,Point(self.pos)])

        # # log everything
        # hdf = pd.HDFStore(self.hdf_path)
        # self.odo_df.to_hdf(hdf,
        #                    key = 'odometer',
        #                    mode = 'a',
        #                    format = 'table',
        #                    append = True,
        #                    )
        # self.batt_df.to_hdf(hdf,
        #                     key = 'battery',
        #                     mode = 'a',
        #                     format = 'table',
        ...
        #                     append = True,)
        # try:
        #     self.jump_df.to_hdf(hdf,'jump',mode = 'a',format = 'table', append = True)
        # except AttributeError:
        #     pass
        # self.swim_df.to_hdf(hdf,'swim',mode = 'a',format = 'table', append = True)
        # self.arb_df.to_hdf(hdf,
        #                    key = 'arbitrate',
        #                    mode = 'a',
        #                    format = 'table',
...
class simulation():
    ...
    def __init__(self, model_dir, model_name, crs):
        ...
        # create dictionary to hold a pandas dataframe of agent positions (value) for every timestep (key)
        self.timestep = {}
    ...
    def enviro_import(self, data_dir, surface_type):
        ...
        read individual rastes into memory mapped files'''
        #raster_cache.get(surface_type, data_dir)
        ...
        if surface_type == 'velocity x':
            ...
            self.vel_x_rast = np.memmap(os.path.join(self.model_dir,'x_vel_memmap.dat'), dtype=dtype, mode='w+', shape=shape)
            self.vel_x_rast_transform = transform
            # map it
            src.read(1, out=self.vel_x_rast)

        elif surface_type == 'velocity y':
            # create a memory mapped array
            self.vel_y_rast = np.memmap(os.path.join(self.model_dir,'y_vel_memmap.dat'), dtype=dtype, mode='w+', shape=shape)
            self.vel_y_rast_transform = transform
            # map it
            src.read(1, out=self.vel_y_rast)

        elif surface_type == 'depth':
            # create a memory mapped array
            self.depth_rast = np.memmap(os.path.join(self.model_dir,'depth_memmap.dat'), dtype=dtype, mode='w+', shape=shape)
            self.depth_rast_transform = transform
            # map it
            src.read(1, out=self.depth_rast)

        elif surface_type == 'wsel':
            # create a memory mapped array
            self.wsel_rast = np.memmap(os.path.join(self.model_dir,'wsel_memmap.dat'), dtype=dtype, mode='w+', shape=shape)
            self.wsel_rast_transform = transform
            # map it
            src.read(1, out=self.wsel_rast)

        elif surface_type == 'elevation':
            # create a memory mapped array
            self.elev_rast = np.memmap(os.path.join(self.model_dir,'elev_memmap.dat'), dtype=dtype, mode='w+', shape=shape)
            self.elev_rast_transform = transform
            # map it
            src.read(1, out=self.elev_rast)

        elif surface_type == 'velocity direction':
            # create a memory mapped array
            self.vel_dir_rast = np.memmap(os.path.join(self.model_dir,'vel_dir_memmap.dat'), dtype=dtype, mode='w+', shape=shape)
            self.vel_dir_rast_transform = transform
            # map it
            src.read(1, out=self.vel_dir_rast)

        elif surface_type == 'velocity magnitude':
            # create a memory mapped array
            self.vel_mag_rast = np.memmap(os.path.join(self.model_dir,'vel_mag_memmat.dat'), dtype=dtype, mode='w+', shape=shape)
            self.vel_mag_rast_transform = transform
            # map it
            src.read(1, out=self.vel_mag_rast)
    ...
    def create_agents(self, numb_agnts, model_dir, starting_box, crs, basin, water_temp):
        ...
        for i in np.arange(0,numb_agnts,1):
            ...
            fishy.initial_heading((self.vel_dir_rast, self.vel_dir_rast_transform))
            fishy.initial_swim_speed()
            fishy.mental_map((self.depth_rast, self.depth_rast_transform))
    ...
    def ts_log(self, ts, pos_dict):
        ...
        timestep = pd.DataFrame.from_dict(pos_dict, orient = 'index', columns = ['E','N','dir','vel','loc'])
        timestep['ts'] = np.repeat(ts,len(timestep))
        timestep.reset_index(inplace = True)
        timestep.rename(columns = {'index':'id'}, inplace = True)
        timestep['E'] = np.round(timestep.E,3)
        timestep['N'] = np.round(timestep.N,3)

        # second step creates a new agents geo dataframe
        self.agents = gpd.GeoDataFrame(columns=['id','E','N','dir','vel','loc'],
                                       geometry='loc',
                                       crs= self.crs)


        gdf = gpd.GeoDataFrame(timestep, geometry='loc', crs= self.crs)
        self.agents = pd.concat([self.agents,gdf])

        self.timestep[ts] = pos_dict


    def divide_agents(self, agents, num_processes):
        """AI generated function that allocates agents into subsets, which are
        ...
        """
        avg = len(agents) // num_processes
        subsets = [agents[i:i+avg] for i in range(0, len(agents), avg)]
        return subsets

    def worker_function(self, subset_of_agents, all_agents, t, dt, pos_dict):
        """AI generated function that passes arguments to the agent specific
        timestep function.

        Parameters
        ----------
        subset_of_agents : TYPE
            DESCRIPTION.
        all_agents : TYPE
            DESCRIPTION.
        t : TYPE
        ...
        """
        depth_tup = (self.depth_rast,self.depth_rast_transform)
        x_vel_tup = (self.vel_x_rast,self.vel_x_rast_transform)
        y_vel_tup = (self.vel_y_rast,self.vel_y_rast_transform)
        vel_dir_tup = (self.vel_dir_rast,self.vel_dir_rast_transform)
        vel_mag_tup = (self.vel_mag_rast,self.vel_mag_rast_transform)


        for agent in subset_of_agents:
            agent.timestep(all_agents, t, dt, pos_dict, depth_tup, x_vel_tup, y_vel_tup, vel_dir_tup, vel_mag_tup)

    def parallel(self, agents, t, dt):
        """Purpose of the parallel method is to partition agents into bins, and
        pass arguments to the agent timestep method
        ----------
        t : TYPE
            DESCRIPTION.
        dt : TYPE
        ...
        """
        # determine number of cores and distribute agents into X bins
        num_cores = multiprocessing.cpu_count()

        agent_subsets = self.divide_agents(agents = self.agents_list,
        ...
        for subset in agent_subsets:
            p = multiprocessing.Process(target = self.worker_function,
                                        args = (subset, self.agents, t, dt, pos_dict))
            processes.append(p)
            p.start()
    ...
    def run(self, model_name, agents, n, dt):
        ...
        # get depth raster
        depth = rasterio.MemoryFile()
        height = self.depth_rast.shape[0]
        width = self.depth_rast.shape[1]
        ...
        with depth.open(
            ...
            dataset.write(self.depth_rast, 1)
            ...
            with writer.saving(fig, os.path.join(self.model_dir,'%s.mp4'%(model_name)), 300):
                for i in range(n):
                    pos_dict = self.parallel(agents, i, dt)

                    # write frame
                    agent_pts.set_data(self.agents.geometry.x.values,
                                       self.agents.geometry.y.values)
                    writer.grab_frame()

                    # log data to hdf
                    self.ts_log(i, pos_dict)

                    print ('Time Step %s complete'%(i))

                for agent in agents:
                    agent.mental_map_export((self.depth_rast, self.depth_rast_transform))
        ...
        depth.close()
        t1 = time.time()
        print ('simulation complete, took %s seconds to compile, check output'%(t1 - t0))
