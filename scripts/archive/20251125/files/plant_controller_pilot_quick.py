"""Pilot plant×controller quick sweep: ndelta_mult x Kp grid with Kd fixed.

Writes `sweep_results/plant_controller_pilot_quick.csv` and per-run traces.
"""
import os
import math
import time
...
try:
    for mult, Kp in itertools.product(NDELTA_MULTS, KP_LIST):
        tag = f'nd{mult:.2f}_Kp{Kp:.3f}_Kd{KD:.3f}_pilot'
        trace_path = os.path.join(OUT, f'pc_pilot_{tag}.csv')
        ...
        pd.DataFrame(rows).to_csv(os.path.join(OUT, 'plant_controller_pilot_quick.csv'), index=False)
