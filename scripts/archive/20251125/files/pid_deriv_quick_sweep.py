"""Quick PID derivative sweep (reduced) using lead_time=20s to get fast feedback.
Runs a smaller set: Kp in [0.4,0.45], Kd in [0.25,0.4,0.6], deriv_tau in [1.0,2.0,3.5].
"""
import os
import math
import numpy as np
...
for kp in Kp_candidates:
    for kd in Kd_candidates:
        for dtau in deriv_taus:
            ...
            trace = os.path.join(OUT, f"pid_trace_quick_{name}.csv")
            ...
            pd.DataFrame(rows).to_csv(os.path.join(OUT, 'pid_deriv_quick_summary.csv'), index=False)
