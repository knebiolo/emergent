"""Small PID retune grid at Ndelta multiplier = 1.25.

Runs a 3x3 grid (Kp x Kd) around the current candidate and writes
`sweep_results/pid_retune_nd1p25_summary.csv` and per-run traces.
"""
import os
import math
import time
...
try:
    ...
    for Kp, Kd in itertools.product(KP_LIST, KD_LIST):
        ...
        pd.DataFrame(rows).to_csv(os.path.join(OUT, 'pid_retune_nd1p25_summary.csv'), index=False)
