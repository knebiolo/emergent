"""Quick PID retune grid at Ndelta multiplier = 1.25 (shorter runtime for fast feedback).

Runs a 3x3 grid (Kp x Kd) around the current candidate and writes
`sweep_results/pid_retune_nd1p25_summary_quick.csv` and per-run traces.
"""
import os
import math
import time
...
try:
    # set plant-level Ndelta modifier (via config so the sim picks it up on spawn)
    ...
    for Kp, Kd in itertools.product(KP_LIST, KD_LIST):
        ...
        # set controller gains in config (sim.spawn may copy defaults into ship)
        ...
        # set ship-local actuator tau and ensure tuning dict consistent
        try:
        ...
        pd.DataFrame(rows).to_csv(os.path.join(OUT, 'pid_retune_nd1p25_summary_quick.csv'), index=False)
