"""Run expanded full-length PID validation (ndelta=1.25) for an expanded set of gains.

Writes `sweep_results/pid_retune_nd1p25_summary_expand.csv`.
"""
import os
import math
import time
...
# expanded grid
KP_LIST = [0.40, 0.45, 0.50]
KD_LIST = [0.40, 0.50]
CASES = [(kp, kd) for kp in KP_LIST for kd in KD_LIST]
...
try:
    ...
    for Kp, Kd in CASES:
        tag = f'nd{ndelta_mult:.2f}_Kp{Kp:.3f}_Kd{Kd:.3f}_fullx'
        ...
        pd.DataFrame(rows).to_csv(os.path.join(OUT, 'pid_retune_nd1p25_summary_expand.csv'), index=False)
