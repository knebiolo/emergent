"""
Short headless runner with a forced lateral current (eastward) to induce heading corrections.
Writes PID internals to scripts/pid_trace_force_short.csv
"""
import os, sys
...
try:
    ...
    pid_out = os.path.abspath(os.path.join(os.path.dirname(__file__), 'pid_trace_force_short.csv'))
...
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), 'headless_force_short_log.txt'))
...
def forced_current(lon, lat, t):
    # return shape (2,N) where N matches lon/lat input shape expected by simulation
    try:
        # if lon is array-like (per-agent), build matching arrays
        lon_arr = np.asarray(lon)
        n = lon_arr.size
        u = np.full((n,), 0.6)
        v = np.zeros((n,))
        # return shape (n,2) so that .T => (2,n) matches simulation expectations
        return np.column_stack([u, v])
    except Exception:
        # fallback: single row (1,2)
