"""Short debug runner: 60s run of the oscillation candidate with sim.verbose=True
and PID trace enabled. Prints planner debug messages to stdout.
"""
...
c = {
    'name': 'keepKp_lowerKd_dtau2.0_debug',
    'Kp': 0.5, 'Ki': 0.0, 'Kd': 0.2, 'deriv_tau': 2.0, 'release_band_deg': 3.0
}

trace = os.path.join(OUT_DIR, f"pid_trace_osc_{c['name']}_short.csv")
...
sim.spawn()
# enable verbose so planner debug prints occur
sim.verbose = True
...
print('Running debug candidate', c['name'], 'T=', T)
sim.run()
