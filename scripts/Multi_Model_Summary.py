...
#  AgentVisualizer     
    ...
    def process_agents(self, hdf_path):
        base_name = os.path.splitext(os.path.basename(hdf_path))[0]
        output_folder = os.path.dirname(hdf_path)
        tiff_output_path = os.path.join(output_folder, f'{base_name}_Agent_Locations_on_TIFF.tif')
        ...
        with h5py.File(hdf_path, 'r') as hdf, rasterio.open(self.tif_path) as tiff:
            ...
            x_indices = x_data.flatten()
            y_indices = y_data.flatten()

            # Debugging to check coordinates range
            print("X coordinates range:", np.min(x_indices), np.max(x_indices))
            print("Y coordinates range:", np.min(y_indices), np.max(y_indices))
            print("TIFF bounds:", tiff.bounds)

            # Plot all data points
            ax.scatter(x_indices, y_indices, s=1, color='red', alpha=0.5)

            ax.set_title('Agent Locations on TIFF')
            plt.xlabel('Easting (E)')
            plt.ylabel('Northing (N)')

            # Save the figure as a TIFF file with potentially lower dpi if high resolution is not needed
            plt.savefig(tiff_output_path, dpi=150)
            plt.close(fig)

            print(f"Agent locations plotted and saved to {tiff_output_path}")
