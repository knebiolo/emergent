...
class Summarization:
    ...
    def length_statistics(self):
        if self.lengths:
            ...
            print("No 'length' values found for statistics")
    ...
    def max_bout_no(self):
        for filename in self.h5_files:
            full_path = os.path.join(self.directory_path, filename)  # Corrected line
            with pd.HDFStore(full_path, mode='r') as store:
                if '/battery' in store:
                    battery_data = pd.read_hdf(full_path, '/battery')
    ...
    def plot_agent_timestep_locations_on_tiff(self):
        ...
        plt.xlabel('Longitude')
        plt.ylabel('Latitude')
        plt.title('Agent Timestep Locations')
        plt.legend()
    ...
    def plot_agent_timestep_locations(self):
        with pd.HDFStore(self.hdf_path, mode='r') as store:
            ts_key = '/TS'
            if ts_key in store:
                ts_data = pd.read_hdf(self.hdf_path, ts_key)
    ...
    def plot_agent_jump_locations_tif(self):  # Change the function name here
        jump_df_all = self.read_jump_data()
        ts_loc_df = self.read_ts_loc_data()
...
#summarization=Summarization(directory_path, hdf_path, tif_path)
